...
static void
toolbar_toggle_callback (GtkToggleAction * togglebutton, gpointer data)
{
  gboolean state;
  ...
  if (state) {
    ...
    gconf_client_set_bool (gconf_client, "/apps/aisleriot/show_toolbar",
			   FALSE, NULL);
  }
}
...
static void
clickmove_toggle_callback (GtkToggleAction * togglebutton, gpointer data)
{
...
void
redo_set_sensitive (gboolean state)
{
...
static const GtkActionEntry actions[] = {
  ...
  {"Select", GTK_STOCK_INDEX, N_("_Select Game..."), "<Ctrl>o",
   N_("Play a different game"), G_CALLBACK (show_select_game_dialog)},
  {"RecentlyPlayed", NULL, N_("Recently _Played")},
  {"Statistics", GTK_STOCK_ADD, N_("S_tatistics"), NULL,
   N_("Show gameplay statistics"), G_CALLBACK (show_statistics_dialog)},
  ...
  {"LeaveFullscreen", GAMES_STOCK_LEAVE_FULLSCREEN, NULL, NULL, NULL,
   G_CALLBACK (fullscreen_callback)},
  {"Cards", NULL, N_("_Cards..."), NULL, NULL,
  ...
  {"Hint", GAMES_STOCK_HINT, NULL, NULL, N_("Show a hint"),
   G_CALLBACK (show_hint_dialog)},
  {"Contents", GAMES_STOCK_CONTENTS, NULL, NULL,
   N_("View help for Aisleriot"), G_CALLBACK (general_help)},
  {"Help", GTK_STOCK_HELP, NULL, "<Shift>F1", N_("View help for this game"),
   G_CALLBACK (help_on_specific_game)},
  {"About", GTK_STOCK_ABOUT, NULL, NULL, NULL,
   G_CALLBACK (help_about_callback)},
};

static const GtkToggleActionEntry toggles[] = {
  {"Toolbar", NULL, N_("_Toolbar"), NULL, "Show or hide the toolbar",
   G_CALLBACK (toolbar_toggle_callback)},
  {"ClickToMove", NULL, N_("_Click to Move"), NULL,
   "Pick up and drop cards by clicking",
   G_CALLBACK (clickmove_toggle_callback)},
};
...
make_option_gconf_key (void)
{
  ...
  if (schema == NULL) {
    ...
    gconf_schema_set_long_desc (schema,
				"An integer encoding a list of boolean values (LSB = first item) for use as options in an solitaire game.");
...
expand_options_from_int (SCM options_list, guint bits)
{
  ...
  for (i = 0; i < l; i++) {
    entry = scm_list_ref (options_list, SCM_MAKINUM (i));
    if (SCM_NFALSEP (scm_list_p (entry))) {
      scm_list_set_x (entry, SCM_MAKINUM (1),
...
void
option_list_set_sensitive (void)
{
  if (menuaction == NULL)
    return;
...
static void
save_option_list (SCM options_list)
{
  gchar *keyname;
  ...
  gconf_client_set_int (gconf_client, keyname,
			compress_options_to_int (options_list), NULL);

  g_free (keyname);
}

static void
load_option_list (SCM options_list)
{
  gchar *keyname;
  guint r;
  ...
  r = gconf_client_get_int (gconf_client, keyname, NULL);
  ...
  if (cscmi_has_options ()) {
	...
	if (mode == menu_normal) {
	  ...
	  gtk_radio_action_set_group (GTK_RADIO_ACTION (itemaction),
				      radiogroup);
	  radiogroup =
	    gtk_radio_action_get_group (GTK_RADIO_ACTION (itemaction));
	}
...
install_recently_played_menu ()
{
  ...
  GtkAction *action =
    gtk_action_group_get_action (action_group, "RecentlyPlayed");
  if (action) {
  ...
  GtkWidget *recent_menu =
    gtk_ui_manager_get_widget (ui_manager,
			       "/MainMenu/GameMenu/RecentlyPlayed");
  ...
  while (game_file_name != NULL) {
    ...
    g_signal_connect (G_OBJECT (pAction), "activate",
		      G_CALLBACK (new_recent_game_action), callback_data);
