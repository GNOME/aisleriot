...
(define (add-to-sequence? row)
  (if (empty-slot? (car (vector-ref rows row)))
      (if (= 0 (modulo (car (vector-ref rows row)) 13))
          (list 0 (string-append (_"Place a two in the leftmost slot of row ")
                                 (number->string (+ row 1))
                                 (_".")
                  )
          )
          (if (not (= 12 (modulo (car (vector-ref rows row)) 13)))
            (list 0 (string-append (_"Add to the sequence in row ")
                                   (number->string (+ row 1))
                                   (_".")
                    )
            )
...
(define (playable-gap? slotlist)
  (if (null? slotlist)
      ...
      (if (and (empty-slot? (car slotlist))
           ...
           (list 0 (string-append 
                     (_"Place the ")
                     (get-value-name (+ 1 (get-value(get-top-card
                       (- (car slotlist) 1))))
                     )
                     (_" of ") 
                     (get-suit-name(get-suit(get-top-card(- (car slotlist) 1))))
                     (_" next to the ")
                     (get-name(get-top-card(- (car slotlist) 1))) (_".")
                   )
           )
