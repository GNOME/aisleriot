...
(define (button-released start-slot card-list end-slot)
  (and (droppable? start-slot card-list end-slot)
       (if (> end-slot 4)
           (begin
             (or (empty-slot? end-slot)
                 (not (= (get-suit (get-top-card end-slot))
...
(define (check-for-discard slot)
   (if (= slot 12)
       #f
       (let ((bottom-thirteen (last-thirteen (get-cards slot))))
         (if (and (= (length bottom-thirteen) 13)
                  (check-same-suit-list bottom-thirteen)
                  (check-straight-descending-list bottom-thirteen)
                  (is-visible? (car (reverse bottom-thirteen)))
             )
             (list 0 (_"Discard the completed pile"))     
             (check-for-discard (+ 1 slot))
         )
       )
   )
)

(define (last-thirteen card-list)
  (if (> (length card-list) 13)
      (last-thirteen (reverse (cdr (reverse card-list))))
      card-list
  )
)
