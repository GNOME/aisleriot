...
static void
select_rules (GtkTreeSelection * select, gpointer data)
{
  GtkTreeModel *model;
  GtkTreeIter iter;
...
static void
select_game (GtkWidget * app, gint response, gpointer data)
{
...
static void
build_list (gchar * filename, GtkWidget * list)
{
  gchar *text;
  GtkTreeIter iter;
  ...
  gtk_list_store_set (GTK_LIST_STORE (list), &iter, 0, text, 1, filename, -1);
...
show_select_game_dialog (void)
{
  ...
  if (gtk_tree_selection_get_selected (select, NULL, &iter)) {
    ...
    gtk_tree_view_scroll_to_cell (GTK_TREE_VIEW (list_view), path, NULL, TRUE,
				  0.5, 0.0);
...
show_hint_dialog ()
{
  ...
  if (hint_dlg) {
    g_signal_connect (GTK_OBJECT (hint_dlg),
		      "destroy", (GtkSignalFunc) hint_destroy_callback, NULL);
  }
...
show_preferences_dialog ()
{
  ...
  if (!property_box) {
    ...
    gtk_window_set_transient_for (GTK_WINDOW (property_box),
				  GTK_WINDOW (app));
...
void
show_global_stats_dialog ()
{
}

void
show_rules_stats_dialog ()
{
}
