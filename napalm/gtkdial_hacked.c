...
/* Slightly hacked by timg for napalm */
...
gtk_dial_update_mouse (GtkDial *dial, gint x, gint y)
{
  ...
  if (dial->angle > 6.*M_PI/6.)
    dial->angle = 6.*M_PI/6.;

  dial->adjustment->value = dial->adjustment->lower + (6.*M_PI/6 - dial->angle) *
    (dial->adjustment->upper - dial->adjustment->lower) / (3.*M_PI/3.);
...
gtk_dial_update (GtkDial *dial)
{
  ...
  dial->angle = 6.*M_PI/6. - (new_value - dial->adjustment->lower) * 3.*M_PI/3. /
